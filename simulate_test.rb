...
config.each do |nSF|
	...
	while duration < 31 do
		...
		if mode == 4.to_s
			...
			while i < 4 do
				...
				if valid_float?(first_line)
					results << first_line.to_f
				end
	...
	if !(avg == 'no data')
		stdev = standard_deviation results
	end
	...
	if mode == 4.to_s
		output_name = ('mode=' << mode << '_ber=' << ber << '_nSF=' << nSF.to_s << '_meanOnDuration=' << meanONDuration << '.txt' ) 
	else
		output_name = ('mode=' << mode << '_ber=' << ber << '_nSF=' << nSF.to_s << '.txt' )
	end

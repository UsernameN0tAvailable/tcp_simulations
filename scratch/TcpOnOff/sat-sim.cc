...
NS_LOG_COMPONENT_DEFINE("TcpOnOff");
...
int main(int argc, char *argv[]) {
    ...
    LogComponentEnable("TcpOnOff", LOG_LEVEL_ERROR);
    ...
    // App attributes
    double totDataRate = 20; // tot data rate per terminal in Mbps
    std::string appOnTime = "ns3::ConstantRandomVariable[Constant=0.5]"; // App On phase duration
    std::string appOffTime = "ns3::ConstantRandomVariable[Constant=2.2]";// App Off phase duration
    ...
    /*
     * Compute data rate per TCP connection
     */
    double ratePerConn = totDataRate/connPerTerminal;
    std::stringstream ss;
    ss << std::to_string(ratePerConn) << "Mbps";
    std::string appDataRate = ss.str();
    ...
    // TCP socket port
    uint16_t port = 999;
    // Amount of data to be set, Zero means infinite.
    uint32_t maxBytes = 0;
    ...
    for (int i = 0; i < connPerTerminal; i++ ) {
        ...
        // First terminal apps
        OnOffHelper oo("ns3::TcpSocketFactory", InetSocketAddress(allIpv4Interfaces[connectionsCount - 1].GetAddress(1), port));
        oo.SetAttribute("MaxBytes", UintegerValue(maxBytes));
        oo.SetAttribute("PacketSize", UintegerValue(packetSize));
        oo.SetAttribute("DataRate", DataRateValue(DataRate(appDataRate)));
        oo.SetAttribute("OnTime", StringValue(appOnTime));
        oo.SetAttribute("OffTime", StringValue(appOffTime));
        ...
        OnOffHelper oo1("ns3::TcpSocketFactory", InetSocketAddress(allIpv4Interfaces[connectionsCount - 2].GetAddress(1), port));
        oo1.SetAttribute("MaxBytes", UintegerValue(maxBytes));
        oo1.SetAttribute("PacketSize", UintegerValue(packetSize));
        oo1.SetAttribute("DataRate", DataRateValue(DataRate(appDataRate)));
        oo1.SetAttribute("OnTime", StringValue(appOnTime));
        oo1.SetAttribute("OffTime", StringValue(appOffTime));
        allSendApps1[i] = oo1.Install(terminal1Nodes.Get(1));
        allSendApps1[i].Start(Seconds(appStart));
        allSendApps1[i].Stop(Seconds(appStop));

        // Third terminal apps
        OnOffHelper oo2("ns3::TcpSocketFactory", InetSocketAddress(allIpv4Interfaces[connectionsCount - 3].GetAddress(1), port));
        oo2.SetAttribute("MaxBytes", UintegerValue(maxBytes));
        oo2.SetAttribute("PacketSize", UintegerValue(packetSize));
        oo2.SetAttribute("DataRate", DataRateValue(DataRate(appDataRate)));
        oo2.SetAttribute("OnTime", StringValue(appOnTime));
        oo2.SetAttribute("OffTime", StringValue(appOffTime));
        ...
        OnOffHelper oo3("ns3::TcpSocketFactory", InetSocketAddress(allIpv4Interfaces[connectionsCount - 4].GetAddress(1), port));
        oo3.SetAttribute("MaxBytes", UintegerValue(maxBytes));
        oo3.SetAttribute("PacketSize", UintegerValue(packetSize));
        oo3.SetAttribute("DataRate", DataRateValue(DataRate(appDataRate)));
        oo3.SetAttribute("OnTime", StringValue(appOnTime));
        oo3.SetAttribute("OffTime", StringValue(appOffTime));
        allSendApps3[i] = oo3.Install(terminal1Nodes.Get(3));
        allSendApps3[i].Start(Seconds(appStart));
        allSendApps3[i].Stop(Seconds(appStop));

        // Fifth terminal apps
        OnOffHelper oo4("ns3::TcpSocketFactory", InetSocketAddress(allIpv4Interfaces[connectionsCount - 5].GetAddress(1), port));
        oo4.SetAttribute("MaxBytes", UintegerValue(maxBytes));
        oo4.SetAttribute("PacketSize", UintegerValue(packetSize));
        oo4.SetAttribute("DataRate", DataRateValue(DataRate(appDataRate)));
        oo4.SetAttribute("OnTime", StringValue(appOnTime));
        oo4.SetAttribute("OffTime", StringValue(appOffTime));
